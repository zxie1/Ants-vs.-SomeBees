...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        place = self.place
        k = 1
        while k < self.min_range and self.min_range != float('inf'):
            place = place.entrance
            k += 1
        while k >= self.min_range and k <= self.max_range and place.entrance != hive:
            if place.entrance.bees:
                return random_or_none(place.entrance.bees)
            place = place.entrance
            k += 1
        else:
    ...
    def action(self, colony):
        ...
        if self.imposter == False:
            ...
            while place.exit != None:
                ant = place.exit.ant
                if ant:
                    if ant not in self.list:
                        self.list.append(ant)
                        ant.damage = ant.damage * 2
                    if ant.container == True:
                        if ant.ant :
                            if ant.ant not in self.list:
                                ant.ant.damage = ant.ant.damage * 2
                                self.list.append(ant.ant)
                place = place.exit
